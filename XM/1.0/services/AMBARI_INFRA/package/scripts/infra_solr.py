...
class InfraSolr(Script):
    ...
    def configure(self, env, upgrade_type=None):
        ...
        if params.security_enabled:
            kinit_cmd = format("{kinit_path_local} -kt {infra_solr_kerberos_keytab} {infra_solr_kerberos_principal};")
    ...
    def start(self, env, upgrade_type=None):
        ...
        if params.is_support_multi:
            for index, mount in enumerate(params.mounts):
                ...
                infra_solr_port = int(params.infra_solr_port) + index
                infra_solr_jmx_port = int(params.infra_solr_jmx_port) + index
                solr_env_sh = params.infra_solr_conf + '/infra-solr-env' + str(index) + ".sh"
                Execute(
                    format( '{solr_bindir}/solr start -cloud -noprompt  -p ' + str(infra_solr_port) + ' -s {infra_solr_datadir} >> {infra_solr_log} 2>&1'),
                    environment={'SOLR_INCLUDE': solr_env_sh},
