...
def install_flume(first=False):
    ...
    if not os.path.exists('/opt/' + params.version_dir) or not os.path.exists(params.install_dir):
        ...
        Execute('chown -R %s:%s /opt/%s' %
                (params.flume_user, params.flume_group, params.version_dir))
        Execute('chown -R %s:%s %s' %
                (params.flume_user, params.flume_group, params.install_dir))
    ...
    if action == 'config':
        ...
        flume_base = as_user(format(
            "{flume_bin} agent --name {{0}} --conf {{1}} --conf-file {{2}} {{3}} > {flume_log_dir}/{{4}}.out 2>&1"),
                             params.flume_user, env={'JAVA_HOME': params.java_home}) + " &"
