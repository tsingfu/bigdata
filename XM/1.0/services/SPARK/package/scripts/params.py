...
install_dir = config['configurations']['spark-env']['install_dir']
download_url = config['configurations']['spark-env']['download_url']
...
if upgrade_direction == Direction.DOWNGRADE:
    stack_version_unformatted = config['commandParams'][
        'original_stack'].split("-")[1]
...
if has_spark_thriftserver and 'spark-thrift-sparkconf' in config[
    'configurations']:
    ...
    if 'spark-thrift-fairscheduler' in config[
        'configurations'] and 'fairscheduler_content' in config[
        'configurations']['spark-thrift-fairscheduler']:
        spark_thrift_fairscheduler_content = config['configurations'][
            'spark-thrift-fairscheduler']['fairscheduler_content']
...
livy_install_dir = config['configurations']['livy-env']['install_dir']
livy_download_url = config['configurations']['livy-env']['download_url']
livy_filename = livy_download_url.split('/')[-1]
livy_version_dir = livy_filename[:-7]

livy_component_directory = livy_install_dir
livy_conf = '/etc/livy'
livy_log_dir = config['configurations']['livy-env']['livy_log_dir']
livy_pid_dir = status_params.livy_pid_dir
livy_home = livy_install_dir
...
livy_livyserver_port = default('configurations/livy-conf/livy.server.port',
                               8998)
...
spark_llap_enabled = default('/configurations/spark-env/enable_spark_llap', False)
spark_llap_jar_url = default('/configurations/spark-env/spark_llap_jar_url','http://yum.example.com/hadoop/spark-llap/spark-llap_2.10-1.0.7-1.6.jar')
