        ...
        if params.has_livyserver:
            ...
            for livyserver_host in params.livy2_livyserver_hosts:
                try:
                    Execute(
                        format(
                            "curl -s -o /dev/null -w'%{{http_code}}' --negotiate -u: -k http://{livyserver_host}:{livy2_livyserver_port}/sessions | grep 200"),
                        tries=3,
                        try_sleep=1,
                        logoutput=True,
                    ...
                    pass
            if len(params.
                   livy2_livyserver_hosts) > 0 and live_livyserver_host == "":
