...
from setup_livy2 import setup_livy, install_livy
...
class LivyServer(Script):
    ...
    def wait_for_dfs_directories_created(self, dirs):
        ...
        if params.security_enabled:
            Execute(
                format(
                    "{kinit_path_local} -kt {livy_kerberos_keytab} {livy2_principal}"),
        ...
        if not is_empty(dir_path):
            ...
            if WebHDFSUtil.is_webhdfs_available(params.is_webhdfs_enabled,
                ...
                dfs_ret_code = shell.call(
                    format("/opt/hadoop/bin/hdfs --config {hadoop_conf_dir} dfs -test -d " +
                           dir_path),
                    user=params.livy2_user)[0]
