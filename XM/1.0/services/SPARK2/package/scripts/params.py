...
install_dir = config['configurations']['spark2-env']['install_dir']
download_url = config['configurations']['spark2-env']['download_url']
...
spark_history_dir = default(
    '/configurations/spark2-defaults/spark.history.fs.logDirectory',
    "hdfs:///spark2-history")
...
spark_history_server_start = format(
...
spark_hadoop_lib_native = config['configurations']['hadoop-env'][
...
# security settings
if security_enabled:
    spark_principal = spark_kerberos_principal.replace(
        '_HOST', spark_history_server_host.lower())

    if is_hive_installed:
        spark_hive_properties.update({
            'hive.metastore.sasl.enabled': str(config['configurations'][
                'hive-site']['hive.metastore.sasl.enabled']).lower(),
            ...
            config['configurations']['hive-site'][
                'hive.server2.authentication.kerberos.keytab'],
            'hive.server2.authentication': config['configurations'][
                'hive-site']['hive.server2.authentication'],
        })

        hive_kerberos_keytab = config['configurations']['hive-site'][
            'hive.server2.authentication.kerberos.keytab']
        hive_kerberos_principal = config['configurations']['hive-site'][
            'hive.server2.authentication.kerberos.principal'].replace(
...
if 'nm_hosts' in config['clusterHostInfo'] and len(config['clusterHostInfo'][
        'nm_hosts']) == 1:
    # use local mode when there's only one nodemanager
    spark_thrift_master = "local[4]"

if has_spark_thriftserver and 'spark2-thrift-sparkconf' in config[
        'configurations']:
    spark_thrift_sparkconf = config['configurations'][
        'spark2-thrift-sparkconf']
    spark_thrift_cmd_opts_properties = config['configurations']['spark2-env'][
    ...
    if is_hive_installed:
        ...
        spark_hive_properties.update({
            'hive.metastore.client.socket.timeout': config['configurations'][
                'hive-site']['hive.metastore.client.socket.timeout']
        })
        spark_hive_properties.update(config['configurations'][
            'spark2-hive-site-override'])

    if 'spark2-thrift-fairscheduler' in config[
            'configurations'] and 'fairscheduler_content' in config[
                'configurations']['spark2-thrift-fairscheduler']:
        spark_thrift_fairscheduler_content = config['configurations'][
            'spark2-thrift-fairscheduler']['fairscheduler_content']
...
livy_install_dir = config['configurations']['livy2-env']['install_dir']
livy_download_url = config['configurations']['livy2-env']['download_url']
livy_filename = livy_download_url.split('/')[-1]
livy_version_dir = livy_filename.replace('.tar.gz','').replace('.tgz','')
livy2_component_directory = livy_install_dir
livy2_conf = '/etc/livy2'
livy2_log_dir = config['configurations']['livy2-env']['livy2_log_dir']
livy2_pid_dir = status_params.livy2_pid_dir
livy2_home = livy2_component_directory
...
livy2_recovery_dir = default(
    "/configurations/livy2-conf/livy.server.recovery.state-store.url",
    "/livy2-recovery")
...
livy2_log4j_properties = config['configurations']['livy2-log4j-properties'][
    'content']
livy2_spark_blacklist_properties = config['configurations'][
    'livy2-spark-blacklist']['content']

if 'livy.server.kerberos.keytab' in config['configurations']['livy2-conf']:
    livy_kerberos_keytab = config['configurations']['livy2-conf'][
        'livy.server.kerberos.keytab']
else:
    livy_kerberos_keytab = config['configurations']['livy2-conf'][
        'livy.server.launch.kerberos.keytab']
if 'livy.server.kerberos.principal' in config['configurations']['livy2-conf']:
    livy_kerberos_principal = config['configurations']['livy2-conf'][
...
if len(livy2_livyserver_hosts) > 0:
    has_livyserver = True
    if security_enabled:
        livy2_principal = livy_kerberos_principal.replace(
            '_HOST', config['hostname'].lower())

livy2_livyserver_port = default('configurations/livy2-conf/livy.server.port',
                                8999)
...
spark_llap_enabled = default('/configurations/spark2-env/enable_spark_llap', False)
spark_llap_jar_url = default('/configurations/spark2-env/spark_llap_jar_url', 'http://yum.example.com/hadoop/spark-llap/spark-llap_2.11-1.1.3-2.1.jar')
if spark_llap_enabled:
    spark_thrift_cmd_opts_properties = spark_thrift_cmd_opts_properties + ' --conf spark.sql.hive.llap=true '