...
class ServiceCheck(Script):
    def service_check(self, env):
        ...
        path_to_distributed_shell_jar = format("{params.install_dir}/share/hadoop/yarn/hadoop-yarn-applications-distributedshell*.jar")
        ...
        yarn_distrubuted_shell_check_cmd = format(" ".join(
            yarn_distrubuted_shell_check_params))
        ...
        if rm_webapp_addresses is not None and len(rm_webapp_addresses) > 0:
            for rm_webapp_address in rm_webapp_addresses:
                ...
                try:
                    ...
                    Logger.warning(
                        format(
                            "Cluster info is not available from calling {get_cluster_info_cmd}"))

        if active_rm_webapp_address is None:
            raise Fail(
                'Resource Manager state is not available. Failed to determine the active Resource Manager web application address from {0}'.
