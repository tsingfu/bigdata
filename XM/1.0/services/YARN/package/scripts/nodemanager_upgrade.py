...
def _check_nodemanager_startup():
  ...
  '''
    ...
    return_code, yarn_output = shell.checked_call(
        command, user=params.yarn_user)
    ...
    if hostname in yarn_output or nodemanager_address in yarn_output or hostname_ip in yarn_output:
        Logger.info('NodeManager with ID \'{0}\' has rejoined the cluster.'.
                    format(nodemanager_address))
        return
    else:
        raise Fail(
            'NodeManager with ID \'{0}\' was not found in the list of running NodeManagers. \'{1}\' output was:\n{2}'.
            format(nodemanager_address, command, yarn_output))
