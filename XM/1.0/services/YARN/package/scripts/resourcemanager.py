...
class Resourcemanager(Script):
    ...
    def security_status(self, env):
        ...
        if status_params.security_enabled:
            ...
            yarn_site_props = build_expectations(
                'yarn-site', props_value_check, props_empty_check,
                props_read_check)
            ...
            result_issues = validate_security_config_properties(
                security_params, yarn_site_props)
            if not result_issues:  # If all validations passed successfully
                try:
                    ...
                    cached_kinit_executor(
                        ...
                        status_params.yarn_user, security_params['yarn-site'][
                            'yarn.resourcemanager.keytab'], security_params[
                            'yarn-site']['yarn.resourcemanager.principal'],
    ...
    def decommission(self, env):
        ...
        yarn_refresh_cmd = format(
            "{rm_kinit_cmd} {params.hadoop_bin_dir}/yarn --config {conf_dir} rmadmin -refreshNodes")
    ...
    def wait_for_dfs_directory_created(self, dir_path, ignored_dfs_dirs):
        ...
        if not is_empty(dir_path):
            ...
            if dir_path in ignored_dfs_dirs:
                Logger.info("Skipping DFS directory '" + dir_path +
                            "' as it's marked to be ignored.")
                return

            Logger.info("Verifying if DFS directory '" + dir_path +
                        "' exists.")
            ...
            if WebHDFSUtil.is_webhdfs_available(params.is_webhdfs_enabled,
                ...
                dfs_ret_code = shell.call(
                    format(params.hadoop_bin_dir + "/hdfs --config {hadoop_conf_dir} dfs -test -d " +
                           dir_path),
                    user=params.yarn_user)[0]
