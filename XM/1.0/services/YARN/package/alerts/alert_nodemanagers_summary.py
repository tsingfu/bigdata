  ...
  """
    ...
    try:
        if kerberos_principal is not None and kerberos_keytab is not None and security_enabled:
            ...
            live_nodemanagers = json.loads(
                get_value_from_jmx(live_nodemanagers_qry, "LiveNodeManagers",
                                   connection_timeout))
        ...
        if kerberos_principal is not None and kerberos_keytab is not None and security_enabled:
            if response_code in [200, 307] and convert_to_json_failed:
                ...
                raise Exception(
                    "[Alert][NodeManager Health Summary] Getting data from {0} failed with http code {1}".
                    format(str(live_nodemanagers_qry), str(response_code)))
