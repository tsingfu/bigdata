...
infra_solr_role_logsearch = default('configurations/infra-solr-security-json/infra_solr_role_logsearch',
                                    'logsearch_user')
infra_solr_role_logfeeder = default('configurations/infra-solr-security-json/infra_solr_role_logfeeder',
                                    'logfeeder_user')
...
if security_enabled:
    ...
    use_external_solr_with_kerberos = default(
        'configurations/logsearch-common-env/logsearch_external_solr_kerberos_enabled', False)
    if use_external_solr_with_kerberos:
        ...
        logsearch_kerberos_principal = config['configurations']['logsearch-env'][
            'logsearch_external_solr_kerberos_principal'].replace('_HOST', _hostname_lowercase)
        logfeeder_kerberos_keytab = config['configurations']['logfeeder-env']['logfeeder_external_solr_kerberos_keytab']
        logfeeder_kerberos_principal = config['configurations']['logfeeder-env'][
            'logfeeder_external_solr_kerberos_principal'].replace('_HOST', _hostname_lowercase)
    else:
        logsearch_kerberos_keytab = config['configurations']['logsearch-env']['logsearch_kerberos_keytab']
        logsearch_kerberos_principal = config['configurations']['logsearch-env'][
            'logsearch_kerberos_principal'].replace('_HOST', _hostname_lowercase)
        logfeeder_kerberos_keytab = config['configurations']['logfeeder-env']['logfeeder_kerberos_keytab']
        logfeeder_kerberos_principal = config['configurations']['logfeeder-env'][
            'logfeeder_kerberos_principal'].replace('_HOST', _hostname_lowercase)

logsearch_spnego_host = config['configurations']['logsearch-properties']['logsearch.spnego.kerberos.host'].replace(
...
logsearch_service_logs_max_retention = config['configurations']['logsearch-service_logs-solrconfig'][
    'logsearch_service_logs_max_retention']
logsearch_service_logs_merge_factor = config['configurations']['logsearch-service_logs-solrconfig'][
    'logsearch_service_logs_merge_factor']

logsearch_audit_logs_max_retention = config['configurations']['logsearch-audit_logs-solrconfig'][
    'logsearch_audit_logs_max_retention']
logsearch_audit_logs_merge_factor = config['configurations']['logsearch-audit_logs-solrconfig'][
...
if logsearch_use_external_solr:
    ...
    logsearch_solr_ssl_enabled = default('configurations/logsearch-common-env/logsearch_external_solr_ssl_enabled',
                                         False)
...
# System logs
logfeeder_system_messages_content = config['configurations']['logfeeder-system_log-env'][
...
if 'infra-solr-env' in config['configurations'] and security_enabled and not logsearch_use_external_solr:
    acl_infra_solr_principal = get_name_from_principal(
        config['configurations']['infra-solr-env']['infra_solr_kerberos_principal'])
    acl_logsearch_principal = get_name_from_principal(
        config['configurations']['logsearch-env']['logsearch_kerberos_principal'])
...
logsearch_properties = dict(
    logsearch_properties.items() + dict(config['configurations']['logsearch-properties']).items())
...
logsearch_properties['logsearch.solr.metrics.collector.hosts'] = format(
    logsearch_properties['logsearch.solr.metrics.collector.hosts'])
logsearch_properties['logsearch.auth.external_auth.host_url'] = format(
...
logfeeder_default_config_file_names = ['global.config.json'] + ['input.config-%s.json' % (tag) for tag in
                                                                logfeeder_default_services]
...
if logfeeder_system_log_enabled:
    default_config_files = ','.join(['output.config.json',
                                     'input.config-ambari.json'] + logfeeder_default_config_file_names + logfeeder_custom_config_file_names
...
logfeeder_properties = dict(
    logfeeder_properties.items() + dict(config['configurations']['logfeeder-properties']).items())

# load derivated values

logfeeder_properties['logfeeder.metrics.collector.hosts'] = format(
...
smoke_logsearch_cmd = format(
    'curl -k -s -o /dev/null -w "%{{http_code}}" {logsearch_ui_protocol}://{logsearch_server_host}:{logsearch_ui_port}/login.html | grep 200')
...
if solr_on_hdfs_enabled:
    logsearch_service_logs_solrconfig_content = config['configurations']['logsearch-service_logs-solrconfig']['content_hdfs']
    logsearch_audit_logs_solrconfig_content = config['configurations']['logsearch-audit_logs-solrconfig']['content_hdfs']