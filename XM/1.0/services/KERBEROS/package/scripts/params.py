...
ldap_kerberos_container_dn = default('configurations/krb5-config/ldap_kerberos_container_dn','cn=krbContainer,ou=Kerberos,dc=example,dc=com')
ldap_kdc_dn = default('configurations/krb5-config/ldap_kdc_dn','cn=kdc-srv,ou=Control,dc=example,dc=com')
ldap_kadmind_dn = default('configurations/krb5-config/ldap_kadmind_dn','cn=kdc-adm,ou=Control,dc=example,dc=com')
kdc_adminpassword = default('configurations/krb5-config/kdc.adminpassword','example.com')
...
if config is not None:
    ...
    if configurations is not None:
        ...
        if krb5_conf_data is not None:
            ...
            test_keytab_file = get_property_value(
                krb5_conf_data, 'test_keytab_file', test_keytab_file, True,
                None)
...
kdc_host_list = kdc_hosts.split(',')
import hashlib
m = hashlib.md5()
m.update(hostname)
hostname_md5 = m.hexdigest()
index = int(hostname_md5[-2:],16) % len(kdc_host_list)
kdc_first = kdc_host_list.pop(index)
kdc_host_list.insert(0,kdc_first)
