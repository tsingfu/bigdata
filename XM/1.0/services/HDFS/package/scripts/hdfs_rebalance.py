...
class HdfsLine():
    ...
    MEMORY_PATTERN = '(?P<memmult_%d>(?P<memory_%d>(\d+)(.|,)?(\d+)?) (?P<mult_%d>' + "|".join(
        MEMORY_SUFFIX) + '))'
    ...
    PROGRESS_PATTERN = re.compile(
        "(?P<date>.*?)\s+" + "(?P<iteration>\d+)\s+" + MEMORY_PATTERN % (
            1, 1, 1) + "\s+" + MEMORY_PATTERN % (
                2, 2, 2) + "\s+" + MEMORY_PATTERN % (3, 3, 3))
    ...
    def parseMemory(self, memorySize, multiplier_type):
        try:
            ...
            raise AmbariException("Failed to memory value [%s %s]" %
                                  (memorySize, multiplier_type))
    ...
    def __str__(self):
        return "[ date=%s,iteration=%d, bytesAlreadyMoved=%d, bytesLeftToMove=%d, bytesBeingMoved=%d]" % (
            self.date, self.iteration, self.bytesAlreadyMoved,
