...
install_dir = config['configurations']['hadoop-env']['install_dir']
...
mapreduce_libs_path = install_dir + "/share/hadoop/mapreduce/lib/*"
...
jn_edits_dir = config['configurations']['hdfs-site'][
...
fs_checkpoint_dirs = default(
    "/configurations/hdfs-site/dfs.namenode.checkpoint.dir", "").split(',')

dfs_data_dirs = default("/configurations/hdfs-site/dfs.datanode.data.dir", "").strip()

dfs_dirs = []
if not dfs_data_dirs:
    with open('/etc/mtab', "r") as f:
        for line in f:
            if not all and line.startswith('none') or not line.startswith('/dev'):
                continue
            is_sata = False
            fields = line.split()
            device = fields[0]
            mountpoint = fields[1]
            if mountpoint.startswith('/data'):
                with open('/sys/block/' + device.split('/')[-1][:-1] + '/queue/rotational','r') as file:
                        is_sata = file.read()
                if is_sata :
                    dfs_dirs.append( '[DISK]file://' + mountpoint + '/dfs/data')
                else:
                    dfs_dirs.append( '[SSD]file://' + mountpoint + '/dfs/data')
    dfs_data_dirs = ','.join(dfs_dirs)
...
dfs_ha_namenode_standby = default(
    "/configurations/hadoop-env/dfs_ha_initial_namenode_standby", None)
