...
def wait_for_safemode_off(hdfs_binary, afterwait_sleep=0, execute_kinit=False):
  ...
  """
    ...
    Logger.info(
        "Waiting up to {0} minutes for the NameNode to leave Safemode...".
...
def format_namenode(force=None):
    ...
    if not params.dfs_ha_enabled:
        if force:
            ExecuteHadoop(
                'namenode -format -clusterId CID-5bd58e9f-e754-4d01-984b-1298486631b1 ',
            ...
            if not is_namenode_formatted(params):
                Execute(
                    format(
                        "/opt/hadoop/bin/hdfs --config {hadoop_conf_dir} namenode -format -clusterId CID-5bd58e9f-e754-4d01-984b-1298486631b1 -nonInteractive"),
                    user=params.hdfs_user,
...
def is_namenode_formatted(params):
    ...
    for mark_dir in mark_dirs:
        if os.path.isdir(mark_dir):
            marked = True
            Logger.info(
                format("{mark_dir} exists. Namenode DFS already formatted"))
    ...
    if not params.update_exclude_file_only:
        ...
        if params.dfs_ha_enabled:
            ...
            nn_refresh_cmd = format(
                'dfsadmin -fs hdfs://{namenode_rpc} -refreshNodes')
        else:
            nn_refresh_cmd = format(
                'dfsadmin -fs {namenode_address} -refreshNodes')
        ExecuteHadoop(
            nn_refresh_cmd,
            user=hdfs_user,
            conf_dir=conf_dir,
...
def bootstrap_standby_namenode(params, use_path=False):
    ...
    try:
        ...
        for i in range(iterations):
            ...
            code, out = shell.call(
                bootstrap_cmd, logoutput=False, user=params.hdfs_user)
            if code == 0:
...
def is_namenode_bootstrapped(params):
    ...
    for mark_dir in mark_dirs:
        if os.path.isdir(mark_dir):
            ...
            Logger.info(
                format(
                    "{mark_dir} exists. Standby Namenode already bootstrapped"))
            break
...
def is_this_namenode_active():
  ...
  """
    ...
    active_namenodes = [] if len(namenode_states[0]) < 1 else namenode_states[
        0]

    # unwraps [('nn2', 'c6402.ambari.apache.org:50070')]
    standby_namenodes = [] if len(namenode_states[1]) < 1 else namenode_states[
        1]
