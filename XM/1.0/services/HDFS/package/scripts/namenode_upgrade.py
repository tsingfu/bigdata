  ...
  """
    ...
    try:
        ...
        safemode_transition_successful, original_state = reach_safemode_state(
            params.hdfs_user, desired_state, params.dfs_ha_enabled,
            hdfs_binary)
        Logger.info("Transition successful: {0}, original state: {1}".format(
            str(safemode_transition_successful), str(original_state)))
        ...
        message = "Could not enter safemode. Error: {0}. As the HDFS user, call this command: {1}".format(
            str(e), safe_mode_enter_cmd)
        Logger.error(message)
        raise Fail(message)
...
def prepare_upgrade_backup_namenode_dir():
  ...
  """
    ...
    failed_paths = []
    nn_name_dirs = params.dfs_name_dir.split(',')
    backup_destination_root_dir = "{0}/{1}".format(
        params.namenode_backup_dir, params.stack_version_unformatted)
    ...
    for nn_dir in nn_name_dirs:
        ...
        backup_current_folder = "{0}/namenode_{1}/".format(
            backup_destination_root_dir, unique)
    ...
    if len(failed_paths) > 0:
        Logger.error(
            ...
            "writeable and copy the directories on your own. Directories: {1}".
            format(backup_destination_root_dir, ", ".join(failed_paths)))
  ...
  """
    ...
    try:
        ...
        code, out = shell.checked_call(
            finalize_command, logoutput=True, user=params.hdfs_user)
        if out:
            expected_substring = "there is no rolling upgrade in progress"
            if expected_substring not in out.lower():
                Logger.warning('Finalize command did not contain substring: %s'
                               % expected_substring)
        else:
...
def reach_safemode_state(user, safemode_state, in_ha, hdfs_binary):
  ...
  """
    Logger.info("Prepare to transition into safemode state %s" %
                safemode_state)
    import params
    ...
    safemode_check_with_grep = format(
        "{safemode_check_cmd} | grep '{grep_pattern}'")
    ...
    if code == 0 and out is not None:
        ...
        if m and len(m.groups()) >= 1:
            ...
            if original_state == safemode_state:
                ...
                # Make a transition
                command = safemode_base_command + safemode_to_instruction[
                    safemode_state]
                Execute(
                    command,
                    user=user,
...
def get_upgrade_in_progress_marker():
  ...
  :return:
  """
    return os.path.join(Script.get_tmp_dir(),
                        NAMENODE_UPGRADE_IN_PROGRESS_MARKER_FILE)
...
def create_upgrade_marker():
  ...
  """
    ...
    try:
        ...
        Logger.warning("Unable to create NameNode upgrade marker file {0}".
                       format(namenode_upgrade_in_progress_marker))
...
def delete_upgrade_marker():
  ...
  """
    ...
    try:
        ...
        error_message = "Unable to remove NameNode upgrade marker file {0}".format(
            namenode_upgrade_in_progress_marker)
