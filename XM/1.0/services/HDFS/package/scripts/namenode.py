...
class NameNode(Script):
    ...
    """
        import params
        Logger.info(
            "Preparing the NameNodes for a NonRolling (aka Express) Upgrade.")
    ...
    def security_status(self, env):
        ...
        hdfs_site_expectations = build_expectations(
            'hdfs-site', props_value_check, props_empty_check,
            props_read_check)
            ...
            if not result_issues:  # If all validations passed successfully
                try:
                    ...
                    if ('hdfs-site' not in security_params or
                            'dfs.namenode.keytab.file' not in
                            security_params['hdfs-site'] or
                            'dfs.namenode.kerberos.principal' not in
                    ...
                    cached_kinit_executor(
                        ...
                        status_params.hdfs_user, security_params['hdfs-site']
                        ['dfs.namenode.keytab.file'], security_params[
    ...
    def rebalancehdfs(self, env):
        ...
        if params.security_enabled:
            ...
            ccache_file_name = "hdfs_rebalance_cc_" + _md5(
                format("{hdfs_principal_name}|{hdfs_user_keytab}")).hexdigest()
        ...
        def startRebalancingProcess(threshold, rebalance_env):
            rebalanceCommand = format(
                '/opt/hadoop/bin/hdfs --config {hadoop_conf_dir} balancer -threshold {threshold}')
            return as_user(
                rebalanceCommand, params.hdfs_user, env=rebalance_env)
