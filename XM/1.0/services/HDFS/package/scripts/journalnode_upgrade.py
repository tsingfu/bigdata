...
def post_upgrade_check():
  ...
  """
    ...
    nn_data = utils.get_jmx_data(
        nn_address, 'org.apache.hadoop.hdfs.server.namenode.FSNamesystem',
        'JournalTransactionInfo', namenode_ha.is_encrypted(),
...
def ensure_jns_have_new_txn(nodelist, last_txn_id):
  ...
  """
    ...
    jn_uri = default("/configurations/hdfs-site/dfs.namenode.shared.edits.dir",
                     None)
    ...
    for i in range(iterations):
        ...
        for node in nodes:
            ...
            if node in actual_txn_ids and actual_txn_ids[
                    node] and actual_txn_ids[node] >= last_txn_id:
                continue

            url = '%s://%s:%s' % (protocol, node, params.journalnode_port)
            data = utils.get_jmx_data(url, 'Journal-', 'LastWrittenTxId',
                                      params.https_only,
                                      params.security_enabled)
            if data:
