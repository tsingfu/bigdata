...
def safe_zkfc_op(action, env):
  ...
  """
    ...
    zkfc = None
    zkfc_slave = ZkfcSlave()
    if action == "start":
        try:
            zkfc_slave.status(env)
        except ComponentIsNotRunning:
            zkfc_slave.start(env)
...
def initiate_safe_zkfc_failover():
  ...
  """
    ...
    if params.namenode_id == active_namenode_id and params.other_namenode_id == standby_namenode_id:
        ...
        Logger.info(msg)
        code, out = shell.call(
            failover_command, user=params.hdfs_user, logoutput=True)
        Logger.info(
        ...
        if code == 0:
            ...
            code, out = shell.call(
                check_standby_cmd, user=params.hdfs_user, logoutput=True)
            Logger.info(
...
def service(action=None,
  ...
  """
    ...
    if user == "root":
        ...
        cmd = format(
            "source {hadoop_conf_dir}/hadoop-env.sh ; {ulimit_cmd} {hadoop_daemon} --config {hadoop_conf_dir} {action} {name}")
        if options:
            cmd += " " + options
  ...
  """
    ...
    if security_enabled:
        ...
        data, error_msg, time_millis = curl_krb_request(
            ...
            params.smokeuser_principal, nn_address, "jn_upgrade",
    ...
    if data:
        ...
        if data_dict:
            for el in data_dict['beans']:
                if el is not None and el['modelerType'] is not None and el[
                        'modelerType'].startswith(modeler_type):
                    if metric in el:
                        my_data = el[metric]
                        if my_data:
...
def get_hdfs_binary(distro_component_name):
  ...
  """
    import params
    hdfs_binary = "{0}/bin/hdfs".format(params.install_dir)
  ...
  """
    ...
    if params.dfs_ha_enabled and use_specific_namenode:
        dfsadmin_base_command = format(
            "source {params.hadoop_conf_dir}/hadoop-env.sh; {hdfs_binary} dfsadmin -fs hdfs://{params.namenode_rpc}")
    else:
        dfsadmin_base_command = format(
            "source {params.hadoop_conf_dir}/hadoop-env.sh; {hdfs_binary} dfsadmin -fs {params.namenode_address}")
