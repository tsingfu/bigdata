...
def get_tokens():
  ...
  """
    return (HDFS_SITE_KEY, NN_HTTP_ADDRESS_KEY, NN_HTTPS_ADDRESS_KEY,
            NN_HTTP_POLICY_KEY, EXECUTABLE_SEARCH_PATHS, NN_CHECKPOINT_TX_KEY,
            NN_CHECKPOINT_PERIOD_KEY, KERBEROS_KEYTAB, KERBEROS_PRINCIPAL,
            SECURITY_ENABLED_KEY, SMOKEUSER_KEY)
...
def execute(configurations={}, parameters={}, host_name=None):
  ...
  """
    ...
    if CHECKPOINT_TX_MULTIPLIER_WARNING_KEY in parameters:
        checkpoint_txn_multiplier_warning = float(parameters[
    ...
    if CHECKPOINT_TX_MULTIPLIER_CRITICAL_KEY in parameters:
        checkpoint_txn_multiplier_critical = float(parameters[
            CHECKPOINT_TX_MULTIPLIER_CRITICAL_KEY])
    ...
    try:
        if kerberos_principal is not None and kerberos_keytab is not None and security_enabled:
            ...
            last_checkpoint_time = int(last_checkpoint_time_response_json[
                "beans"][0]["LastCheckpointTime"])
        ...
        last_tx = int(journal_transaction_info_dict[
            'LastAppliedOrWrittenTxId'])
        most_recent_tx = int(journal_transaction_info_dict[
        ...
        label = LABEL.format(
            h=get_time(delta)['h'], m=get_time(delta)['m'],
