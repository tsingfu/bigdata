  ...
  """
    ...
    if DFS_DATA_DIR not in configurations:
        return (
            ...
            ['{0} is a required parameter for the script'.format(DFS_DATA_DIR)]
        )
    ...
    if dfs_data_dir is None:
        return (RESULT_STATE_UNKNOWN, [
            '{0} is a required parameter for the script and the value is null'.
            format(DFS_DATA_DIR)
        ])
    ...
    data_dirs_unmounted = [
    ]  # list of data dirs that are known to have become unmounted
    ...
    for data_dir in normalized_data_dirs:
        ...
        if os.path.isdir(data_dir):
            ...
            curr_mount_point = curr_mount_point.strip(
            ) if curr_mount_point else curr_mount_point
    ...
    if data_dirs_unknown:
        errors.append(
            "Cannot find the mount point for the following data dir(s):\n{0}".
    ...
    if data_dir_mount_file_exists:
        ...
        expected_data_dir_to_mount = mounted_dirs_helper.get_dir_to_mount_from_file(
            DATA_DIR_MOUNT_FILE)
        for data_dir in data_dirs_on_root:
            if data_dir in expected_data_dir_to_mount and expected_data_dir_to_mount[
                    data_dir] != "/":
        ...
        if len(data_dirs_on_mount) >= 1 and len(data_dirs_on_root) >= 1:
            errors.append(
                "Detected at least one data dir on a mount point, but these are writing to the root partition:\n{0}".
