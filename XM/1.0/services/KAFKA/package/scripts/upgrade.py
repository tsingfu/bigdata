...
def run_migration(env, upgrade_type):
  ...
  """
    ...
    Logger.info("Upgrade type: {0}, direction: {1}".format(
        str(upgrade_type), params.upgrade_direction))
    ...
    if params.upgrade_direction == Direction.UPGRADE:
        kafka_acls_script = format("{params.install_dir}/bin/kafka-acls.sh")
    ...
    if kafka_acls_script is not None:
        if os.path.exists(kafka_acls_script):
            Logger.info("Found Kafka acls script: {0}".format(
                kafka_acls_script))
            if params.zookeeper_connect is None:
                raise Fail(
                    "Could not retrieve property kafka-broker/zookeeper.connect")
            ...
            Logger.info("Did not find Kafka acls script: {0}".format(
                kafka_acls_script))
