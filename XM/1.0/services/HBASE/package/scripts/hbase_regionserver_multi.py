...
class HbaseRegionServer(Script):
    ...
    def configure(self, env):
        ...
        File(params.hbase_conf_dir + '/hbase-regionserver',
            owner=params.hbase_user,
            content=InlineTemplate(params.regionserver_init_template),
    ...
    def start(self, env, upgrade_type=None):
        ...
        setup_ranger_hbase(upgrade_type=upgrade_type, service_name="hbase-regionserver",only_download=True)

        try:
            Execute('source ' + params.hbase_conf_dir + '/hbase-env.sh; ' + params.hbase_conf_dir + '/hbase-regionserver restart 1 2 ')
        except:
    ...
    def stop(self, env, upgrade_type=None):
        ...
        try:
            Execute('source ' + params.hbase_conf_dir + '/hbase-env.sh; ' + params.hbase_conf_dir + '/hbase-regionserver stop')
        except:
            show_logs(params.log_dir, params.hbase_user)
            raise
    def restart(self, env, upgrade_type=None):
        self.stop(env,upgrade_type)
        self.start(env,upgrade_type)

    def status(self, env):
        import status_params
        Execute('source ' + status_params.hbase_conf_dir + '/hbase-env.sh; ' + status_params.hbase_conf_dir + '/hbase-regionserver status')
    ...
    def security_status(self, env):
        ...
        if status_params.security_enabled:
            ...
            hbase_site_expectations = build_expectations(
                'hbase-site', props_value_check, props_empty_check,
                props_read_check)
            ...
            if not result_issues:  # If all validations passed successfully
                try:
                    ...
                    if ('hbase-site' not in security_params or
                            'hbase.regionserver.keytab.file' not in
                            security_params['hbase-site'] or
                            'hbase.regionserver.kerberos.principal' not in
                    ...
                    cached_kinit_executor(
                        ...
                        status_params.hbase_user, security_params[
    ...
    def get_pid_files(self):
        ...
        # import glob
        # return glob.glob( status_params.pid_dir + '/hbase-hbase-*-regionserver.pid')
