...
install_dir = config['configurations']['hbase-env']['install_dir']
download_url = config['configurations']['hbase-env']['download_url']
filename = download_url.split('/')[-1]
#version_dir = filename[:-7]
version_dir = filename.replace('.tar.gz','').replace('.tgz','')

phoenix_install_dir = config['configurations']['hbase-env'][
    'phoenix_install_dir']
...
hbase_user_nofile_limit = default(
    "/configurations/hbase-env/hbase_user_nofile_limit", "1280000")
...
phx_daemon_script = format('/opt/phoenix-server/bin/queryserver.py')
...
java_io_tmpdir = default("/configurations/hbase-env/hbase_java_io_tmpdir",
                         "/tmp")
master_heapsize = ensure_unit_for_memory(config['configurations']['hbase-env'][
    'hbase_master_heapsize'])

regionserver_heapsize = ensure_unit_for_memory(config['configurations'][
    'hbase-env']['hbase_regionserver_heapsize'])
...
if security_enabled:
    ...
    regionserver_jaas_princ = config['configurations']['hbase-site'][
        'hbase.regionserver.kerberos.principal'].replace('_HOST',
                                                         _hostname_lowercase)
...
regionserver_init_template = config['configurations']['hbase-env']['regionserver_init']
...
if enable_ranger_hbase:
    ...
    elif stack_supports_ranger_kerberos and security_enabled:
