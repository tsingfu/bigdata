...
def turn_on_maintenance():
    ...
    turn_on_url = ambari_server_auth_host_url + '/api/v1/clusters/' + cluster_name + '/services/KRB5'
    data = '''{"RequestInfo": {"context": "Turn On Maintenance Mode for KRB5"},"Body": {"ServiceInfo": {"maintenance_state": "ON"}}}'''
...
class Master(Script):
    def install(self, env):
        ...
        try:
            Execute(
                'kdb5_ldap_util -D ' + params.ldap_kadmind_dn + ' -H ldapi:// create -r ' + params.kdc_realm + ' -s< passwd.txt')
        ...
        Execute('echo >> passwd.txt')
        Execute(
            'kdb5_ldap_util -D ' + params.binddn + ' stashsrvpw -f /var/kerberos/krb5kdc/service.keyfile ' + params.ldap_kdc_dn + '< passwd.txt')
        Execute(
            'kdb5_ldap_util -D ' + params.binddn + ' stashsrvpw -f /var/kerberos/krb5kdc/service.keyfile ' + params.ldap_kadmind_dn + '< passwd.txt')
