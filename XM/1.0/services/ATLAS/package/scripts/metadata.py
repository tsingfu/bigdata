...
def install_atlas(first=False):
    ...
    if not os.path.exists('/opt/' + params.version_dir) or not os.path.exists(params.install_dir):
        ...
        Execute(
            ...
            user=status_params.metadata_user)
        Execute('tar -zxf /tmp/' + params.filename + ' -C /opt')
        Execute('ln -s /opt/' + params.version_dir + ' ' + params.install_dir)
        Execute(' cp -r ' + params.install_dir + '/conf/* ' + status_params.conf_dir)
        Execute(' rm -rf ' + params.install_dir + '/conf')
        Execute('ln -s ' + status_params.conf_dir + ' ' + params.install_dir +
                '/conf')
        Execute("echo 'export PATH=%s/bin:$PATH'>>/etc/profile.d/hadoop.sh" %
                params.install_dir)
        Execute('chown -R %s:%s /opt/%s' %
                (status_params.metadata_user, params.user_group, params.version_dir))
        Execute('chown -R %s:%s %s' %
                (status_params.metadata_user, params.user_group, params.install_dir))
...
def metadata(type='server'):
    ...
    if params.security_enabled:
        ...
        Execute(kinit_cmd, tries=2, try_sleep=3, user=params.metadata_user, logoutput=True)
