...
def titan_service(action='start'):
    ...
    if action == 'start':
        ...
        titan_create_data_dir_command = format(
            "hadoop fs -mkdir -p {titan_hdfs_data_dir}; hadoop fs -chown -R titan:hdfs /user/titan")
        ...
        titan_get_spark_tar_command = format(
            "hadoop fs -get {titan_spark2_archive_dir}/{titan_spark2_archive_file} {titan_ext_spark_plugin_dir}")
        titan_sparktargz_exist_command = format(
            "ls {titan_ext_spark_plugin_dir}/{titan_spark2_archive_file}>/dev/null 2>&1")
        Execute(titan_get_spark_tar_command,
                not_if=titan_sparktargz_exist_command,
                logoutput=True, user=params.titan_user)

        # extract spark targz
        titan_x_spark_targz_command = format(
            "tar -xzvf {titan_ext_spark_plugin_dir}/{titan_spark2_archive_file} -C {titan_ext_spark_plugin_dir}/>/dev/null 2>&1")
        ...
        titan_put_spark_jar_command = format(
            "hadoop fs -put -f {titan_ext_spark_plugin_dir}/* {titan_hdfs_spark_lib_dir}; hadoop fs -rm -r {titan_hdfs_spark_lib_dir}/guava*.jar; hadoop fs -put -f {titan_home_dir}/lib/guava*.jar {titan_hdfs_spark_lib_dir}")
        ...
        titan_rm_conflict_jars_command = format(
            "rm -rf {titan_ext_spark_plugin_dir}/guava*.jar; rm -rf {titan_ext_spark_plugin_dir}/slf4j-log4j12*.jar; rm -rf {titan_ext_spark_plugin_dir}/spark-core*.jar; ")
        ...
        no_op_test = format(
            "ls {params_server.titan_pid_file} >/dev/null 2>&1 && ps `cat {params_server.titan_pid_file}` >/dev/null 2>&1")
        Execute(daemon_cmd,
                not_if=no_op_test,
                user=params.titan_user
                )
