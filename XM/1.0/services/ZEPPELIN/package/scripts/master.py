...
def install_zeppelin():
    ...
    if not os.path.exists('/opt/' + params.version_dir) or not os.path.exists(params.install_dir):
        ...
        Execute('chown -R %s:%s /opt/%s' % (
            params.zeppelin_user, params.zeppelin_group, params.version_dir))
            ...
            if interpreter['group'] == 'jdbc':
                ...
                if params.hive_server_interactive_hosts:
                    interpreter['properties'][
                        hive_interactive_properties_key + '.driver'] = 'org.apache.hive.jdbc.HiveDriver'
                    ...
                    interpreter['properties'][
                        hive_interactive_properties_key + '.proxy.user.property'] = 'hive.server2.proxy.user'
                    if params.hive_server2_support_dynamic_service_discovery:
                ...
                if 'spark-env' in params.config['configurations']:
                    ...
                    interpreter['properties']['SPARK_HOME'] = "/opt/spark"
                else:
                    del interpreter_settings[setting_key]
