...
install_dir = config['configurations']['zeppelin-env']['install_dir']
download_url = config['configurations']['zeppelin-env']['download_url']
...
conf_dir = "/etc/zeppelin"
external_dependency_conf = "/etc/zeppelin/external-dependency-conf"
notebook_dir = os.path.join(*[install_dir, 'notebook'])
...
if 'hive_server_host' in master_configs and len(master_configs['hive_server_host']) != 0:
    ...
    hive_server2_support_dynamic_service_discovery = config['configurations']['hive-site'][
...
if 'hive_server_interactive_hosts' in master_configs and len(master_configs['hive_server_interactive_hosts']) != 0:
    ...
    hive_interactive_zookeeper_namespace = config['configurations']['hive-interactive-site'][
...
if 'spark_thriftserver_hosts' in master_configs and len(master_configs['spark_thriftserver_hosts']) != 0:
    ...
    if config['configurations']['spark-thrift-sparkconf'] and \
                    'spark.sql.hive.hiveserver2.jdbc.url.principal' in config['configurations'][
                'spark-thrift-sparkconf']:
        spark_hive_principal = config['configurations']['spark-thrift-sparkconf'][
...
if 'spark2_thriftserver_hosts' in master_configs and len(master_configs['spark2_thriftserver_hosts']) != 0:
    ...
    if config['configurations']['spark2-thrift-sparkconf'] and \
        ...
        spark2_hive_principal = config['configurations']['spark2-thrift-sparkconf'][
            'spark.sql.hive.hiveserver2.jdbc.url.principal']
...
elif 'spark2-defaults' in config['configurations'] and 'spark.yarn.queue' in config['configurations'][
    'spark2-defaults']:
