    ...
    if name.find('_JAAS_PLACEHOLDER') > -1:
        if security_enabled:
            return name.replace('_JAAS_PLACEHOLDER',
                                ' -Djava.security.auth.login.config=' + conf_dir
                                + '/storm_jaas.conf')
        else:
            return name.replace('_JAAS_PLACEHOLDER', '')
    else:
...
def yaml_config_template(configurations):
    return InlineTemplate(
        ...
        extra_imports=[escape_yaml_property, replace_jaas_placeholder,
...
def yaml_config(filename,
    ...
    config_content = InlineTemplate(
        '''{% for key, value in configurations_dict|dictsort %}{{ key }}: {{ escape_yaml_property(resource_management.core.source.InlineTemplate(value).get_content()) }}
{% endfor %}''',
    ...
    File(
        os.path.join(params.conf_dir, filename),
