  ...
  """
    ...
    """
        ...
        if storm_zookeeper_root_dir is None:
            raise Fail(
                "The storm ZooKeeper directory specified by storm-site/storm.zookeeper.root must be specified")

        # The zookeeper client must be given a zookeeper host to contact. Guaranteed to have at least one host.
        storm_zookeeper_server_list = yaml_utils.get_values_from_yaml_array(
            params.storm_zookeeper_servers)
        if storm_zookeeper_server_list is None:
            Logger.info(
                "Unable to extract ZooKeeper hosts from '{0}', assuming localhost").format(
                    params.storm_zookeeper_servers)
        ...
        for storm_zookeeper_server in storm_zookeeper_server_list:
            ...
            zk_command_location = os.path.join("/opt/zookeeper", "bin",
                                               "zkCli.sh")
            ...
            try:
                ...
                if params.security_enabled:
                    env_map[
                        'JVMFLAGS'] = "  -Djava.security.auth.login.config={0}".format(
                            params.storm_jaas_file)
        ...
        if not zookeeper_data_cleared:
            raise Fail(
                "Unable to clear ZooKeeper Storm data on any of the following ZooKeeper hosts: {0}".
                format(storm_zookeeper_server_list))
    ...
    """
        ...
        if os.path.isdir(storm_local_directory):
            ...
            Directory(
                storm_local_directory, action="delete", create_parents=True)
