...
stack_supports_storm_kerberos = True
...
if 'ganglia_server_host' in config['clusterHostInfo'] and len(config[
                                                                  'clusterHostInfo']['ganglia_server_host']) > 0:
...
storm_user_nofile_limit = default(
    '/configurations/storm-env/storm_user_nofile_limit', 1280000)
...
sink_jar = metric_collector_sink_jar

# storm 0.x version use metric_collector_legacy_sink_jar
if version_dir.split('-')[-1].startswith('0'):
    sink_jar = metric_collector_legacy_sink_jar
...
if enable_ranger_storm:
    ...
    common_name_for_certificate = config['configurations']['ranger-storm-plugin-properties'][
    ...
    if has_ranger_admin and stack_supports_ranger_audit_db:
        ...
        driver_curl_target = format(
            "{storm_component_home_dir}/lib/{jdbc_jar_name}") if stack_supports_ranger_audit_db else None
        previous_jdbc_jar = format(
            "{storm_component_home_dir}/lib/{previous_jdbc_jar_name}") if stack_supports_ranger_audit_db else None
        sql_connector_jar = ''
    ...
    ssl_truststore_password = config['configurations']['ranger-storm-policymgr-ssl'][
...
#     atlas_conf_dir = format('{stack_root}/atlas-server/conf')
