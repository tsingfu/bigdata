...
class RangerUsersync(Script):
    def install_ranger(self,first=False):
        ...
        Directory(
            ...
            mode=0775,
            create_parents=True)
        if not os.path.exists('/opt/' + params.version_dir_usersync) or not os.path.exists(params.install_dir_usersync):
            Execute('rm -rf %s' % params.install_dir_usersync)
            Execute(
                'wget ' + params.download_url_usersync + ' -O /tmp/' + params.filename_usersync,
                user=params.unix_user)
            Execute('tar -zxf /tmp/' + params.filename_usersync + ' -C /opt')
            Execute('ln -s /opt/' + params.version_dir_usersync + ' ' + params.install_dir_usersync)
            Execute(' rm -rf ' + params.install_dir_usersync + '/conf')
            Execute('ln -s ' + params.ranger_ugsync_conf + ' ' + params.install_dir_usersync +
                    '/conf')
            Execute('ln -s ' + params.usersync_log_dir + ' ' + params.install_dir_usersync +
                    '/logs')
            Execute('chown -R %s:%s /opt/%s' %
                    (params.unix_user, params.user_group, params.version_dir_usersync))
            Execute('chown -R %s:%s %s' %
                    (params.unix_user, params.user_group, params.install_dir_usersync))
    ...
    def start(self, env, upgrade_type=None):
        ...
        Execute("chown -R %s:%s %s" %(params.unix_user,params.unix_group,params.install_dir_usersync))
        Execute("chown -R %s:%s %s" %(params.unix_user,params.unix_group,params.ranger_ugsync_conf))

        if params.security_enabled:
            kinit_cmd = format("{kinit_path_local} -kt {ugsync_keytab} {ugsync_principal};")
    ...
    def stop(self, env, upgrade_type=None):
        ...
        Execute((params.usersync_services_file, ' stop'), environment={'JAVA_HOME': params.java_home}, sudo=True)
