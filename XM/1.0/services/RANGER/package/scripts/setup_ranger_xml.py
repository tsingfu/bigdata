...
def do_keystore_setup(upgrade_type=None):
    ...
    if not is_empty(params.ranger_credential_provider_path):
        ranger_credential_helper(cred_lib_path, params.ranger_jpa_jdbc_credential_alias,
                                 params.ranger_ambari_db_password, params.ranger_credential_provider_path)
    ...
    params.ranger_audit_source_type).lower() == 'db' and not is_empty(params.ranger_ambari_audit_db_password):
        ranger_credential_helper(cred_lib_path, params.ranger_jpa_audit_jdbc_credential_alias,
                                 params.ranger_ambari_audit_db_password, params.ranger_credential_provider_path)
    ...
    if params.ranger_auth_method.upper() == "LDAP":
        ranger_credential_helper(params.cred_lib_path, params.ranger_ldap_password_alias,
                                 params.ranger_usersync_ldap_ldapbindpassword, params.ranger_credential_provider_path)
    ...
    Execute('chown -R ranger:%s %s' % (params.user_group,ranger_home))
...
def copy_jdbc_connector(stack_version=None):
    ...
    Execute(
        ('cp', '/usr/share/java/mysql-connector-java.jar', os.path.join(ranger_home, 'ews', 'lib')),
        path=["/bin", "/usr/bin/"],
        sudo=True)

    Execute(
        ('cp', '/usr/share/java/mysql-connector-java.jar', os.path.join(ranger_home, 'jisql', 'lib')),
        path=["/bin", "/usr/bin/"],
        sudo=True)
...
def setup_ranger_admin_passwd_change():
    ...
    if params.admin_password != params.default_admin_password:
        try:
            cmd = format(
                'ambari-python-wrap {ranger_home}/db_setup.py -changepassword {admin_username} {default_admin_password!p} {admin_password!p}')
            Logger.info('Updating admin password')
            Execute(cmd, environment={'JAVA_HOME': params.java_home, 'RANGER_ADMIN_HOME': params.ranger_home},
                    user=params.unix_user)
        except Exception as e:
            Logger.error("chage admin passwd fail")
