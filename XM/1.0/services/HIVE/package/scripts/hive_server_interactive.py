...
class HiveServerInteractive(Script):
    ...
    def start(self, env, upgrade_type=None):
        ...
        hive_service_interactive(
            'hiveserver2', action='start', upgrade_type=upgrade_type)
    ...
    def _llap_stop(self, env):
        ...
        stop_cmd = ["/opt/slider/bin/slider", "stop", params.llap_app_name]
        ...
        Execute(('/opt/slider/bin/slider', 'destroy', params.llap_app_name, "--force"),
    ...
    def _llap_start(self, env, cleanup=False):
        ...
        if params.hive_server_interactive_ha:
            ...
            if params.stack_supports_hive_interactive_ga:
                status = self.check_llap_app_status_in_llap_ga(params.llap_app_name, 2,
                                                               params.hive_server_interactive_ha)
            else:
                status = self.check_llap_app_status_in_llap_tp(params.llap_app_name, 2,
                                                               params.hive_server_interactive_ha)
        ...
        cmd = format(
            "source /etc/hive/hive-env.sh; export HIVE_AUX_JARS_PATH=\"\"; {install_dir}/bin/hive --service llap --slider-am-container-mb {params.slider_am_container_mb} "
        ...
        if params.stack_supports_hive_interactive_ga:
            ...
            if long(params.llap_daemon_container_size) > (0.5 * long(params.yarn_nm_mem)):
                ...
                Logger.info("Setting slider_placement : 0, as llap_daemon_container_size : {0} > 0.5 * "
                            "YARN NodeManager Memory({1})".format(params.llap_daemon_container_size,
                                                                  params.yarn_nm_mem))
            else:
        ...
        try:
            ...
            code, output, error = shell.checked_call(cmd, user=params.hive_user, quiet=True, stderr=subprocess.PIPE,
                                                     logoutput=True)
            ...
            if not run_file_path:
                raise Fail("Did not find run.sh file in path: " + str(output))
    ...
    def setup_security(self):
        ...
        slider_keytab_install_cmd = format(
            "/opt/slider/bin/slider install-keytab --keytab {params.hive_llap_keytab_file} --folder {params.hive_user} --overwrite")
    ...
    def _get_llap_app_status_info_in_llap_tp(self, app_name):
        ...
        llap_status_cmd = format(
            "{install_dir}/bin/hive --service llapstatus --name {app_name} --findAppTimeout {LLAP_APP_STATUS_CMD_TIMEOUT}")
    ...
    def _get_llap_app_status_info_in_llap_ga(self, percent_desired_instances_to_be_up, total_timeout, refresh_rate):
        ...
        llap_status_cmd = format(
            "{install_dir}/bin/hive --service llapstatus -w -r {percent_desired_instances_to_be_up} -i {refresh_rate} -t {total_timeout}")
        Logger.info("\n\n\n\n\n");
        Logger.info("LLAP status command : {0}".format(llap_status_cmd))
        code, output, error = shell.checked_call(llap_status_cmd, user=status_params.hive_user, quiet=True,
    ...
    def check_llap_app_status_in_llap_ga(self, llap_app_name, num_retries, return_immediately_if_stopped=False):
        ...
        Logger.debug(
            "Calculated 'total_timeout' : {0} using config 'num_retries_for_checking_llap_status' : {1}".format(
        ...
        llap_app_info = self._get_llap_app_status_info_in_llap_ga(percent_desired_instances_to_be_up / 100.0,
            ...
            if percentInstancesUp >= percent_desired_instances_to_be_up:
                ...
                Logger.info("LLAP app '{0}' in '{1}' state. Live Instances : '{2}'. Desired Instances : " \
                            "'{3}' after {4} secs.".format(llap_app_name, llap_app_info['state'],
                                                           ...
                                                           llap_app_info['desiredInstances'],
                                                           time.time() - curr_time))
                raise Fail("App state is RUNNING_PARTIAL. Live Instances : '{0}', Desired Instance : '{1}'".format(
                    llap_app_info['liveInstances'],
                    llap_app_info['desiredInstances']))
