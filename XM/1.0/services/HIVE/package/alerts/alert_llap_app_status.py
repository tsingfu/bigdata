...
def get_tokens():
  ...
  """
    return (SECURITY_ENABLED_KEY, KERBEROS_EXECUTABLE_SEARCH_PATHS_KEY,
            HIVE_PRINCIPAL_KEY, HIVE_PRINCIPAL_KEYTAB_KEY, HIVE_USER_KEY,
            STACK_ROOT, LLAP_APP_NAME_KEY)
    ...
    try:
        ...
        if STACK_ROOT in configurations:
            llap_status_cmd = format(
                "/opt/hive/bin/hive --service llapstatus --name {llap_app_name}  --findAppTimeout {LLAP_APP_STATUS_CMD_TIMEOUT}")
        else:
            llap_status_cmd = format(
                "/opt/hive/bin/hive --service llapstatus --name {llap_app_name} --findAppTimeout {LLAP_APP_STATUS_CMD_TIMEOUT}")
...
def make_valid_json(output):
  ...
  '''
    ...
    for idx, split in enumerate(splits):
        ...
        if curr_elem == "{":
            if next_elem == "\"amInfo\" : {" and (splits[len_splits - 1]
                                                  ).strip() == '}':
                # For Case 'A'
                marker_idx = idx
                break
            elif idx + 3 == len_splits and next_elem.startswith(
                    '"state" : ') and (splits[idx + 2]).strip() == '}':
                # For Case 'B'
                marker_idx = idx
                break
