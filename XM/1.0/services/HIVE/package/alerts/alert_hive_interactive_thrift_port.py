...
def get_tokens():
  ...
  """
    return (HIVE_SERVER_INTERACTIVE_THRIFT_PORT_KEY, SECURITY_ENABLED_KEY,
            SMOKEUSER_KEY, HIVE_SERVER2_INTERACTIVE_AUTHENTICATION_KEY,
...
def execute(configurations={}, parameters={}, host_name=None):
  ...
  """
    ...
    if HIVE_SERVER_INTERACTIVE_TRANSPORT_MODE_KEY in configurations:
        transport_mode = configurations[
            HIVE_SERVER_INTERACTIVE_TRANSPORT_MODE_KEY]

    port = THRIFT_PORT_DEFAULT
    if transport_mode.lower(
    ) == 'binary' and HIVE_SERVER_INTERACTIVE_THRIFT_PORT_KEY in configurations:
        port = int(configurations[HIVE_SERVER_INTERACTIVE_THRIFT_PORT_KEY])
    elif transport_mode.lower(
    ) == 'http' and HIVE_SERVER_INTERACTIVE_THRIFT_HTTP_PORT_KEY in configurations:
        port = int(configurations[
                       HIVE_SERVER_INTERACTIVE_THRIFT_HTTP_PORT_KEY])
