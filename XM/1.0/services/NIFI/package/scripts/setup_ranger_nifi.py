...
def setup_ranger_nifi(upgrade_type=None):
    ...
    if params.has_ranger_admin and params.enable_ranger_nifi:
        ...
        cred_setup_prefix_path = (
            format('{stack_root}/{stack_version}/{service_name}/ext/ranger/scripts/ranger_credential_helper.py'), '-l',
            cred_lib_prefix_path)
        ...
        setup_ranger_plugin('nifi', params.service_name, params.previous_jdbc_jar,
                            ...
                            component_user=params.nifi_user, component_group=params.nifi_group,
                            cache_service_list=['nifi'],
                            plugin_audit_properties=params.config['configurations']['ranger-nifi-audit'],
                            plugin_audit_attributes=params.config['configuration_attributes']['ranger-nifi-audit'],
                            plugin_security_properties=params.config['configurations']['ranger-nifi-security'],
        ...
        File(os.path.join(params.nifi_config_dir, 'ranger-nifi-audit.xml'), owner=params.nifi_user,
             group=params.nifi_group, mode=0400)
        File(os.path.join(params.nifi_config_dir, 'ranger-nifi-security.xml'), owner=params.nifi_user,
             group=params.nifi_group, mode=0400)
        File(os.path.join(params.nifi_config_dir, 'ranger-policymgr-ssl.xml'), owner=params.nifi_user,
             group=params.nifi_group, mode=0400)
