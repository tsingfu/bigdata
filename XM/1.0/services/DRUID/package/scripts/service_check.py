...
class ServiceCheck(Script):
    ...
    def checkComponent(self, params, component_name, config_name):
        component_port = params.config['configurations'][format(
            '{config_name}')]['druid.port']
        for component_host in params.config['clusterHostInfo'][format(
                '{component_name}_hosts')]:
            Execute(
                format(
