...
if not ipa_server:
    raise KeyError(
        'ipa_server could not be found in this cluster, this is strange and indicates much worse problems'
...
directory_password = default('configurations/freeipa/directory_password',
                             False)
if not directory_password or len(directory_password) < 8:
    raise Exception(
        'FreeIPA directory_password: \'%s\' isn\'t acceptable (min 8 char long)'
        % directory_password)
else:
...
ldap_bind_user = default('configurations/freeipa/ldap_bind_user',
                         'kave_bind_user')
...
kadm5acl_template = default('configurations/freeipa/kadm5acl_template',
                            """*/admin@{{realm}} *
admin@{{realm}} *
admin@{{realm}} a *
admin@{{realm}} i *
...
resolvconf_template = default('configurations/freeipa/resolvconf_template',
                              """search {{domain}}
...
for user, passwd in initial_user_passwords.iteritems():
    if len(passwd) < 8:
        raise ValueError(
            "User : " + user +
            " cannot be assigned an intital password less than 8 characters")
    else:
        Logger.sensitive_strings[passwd] = "[PROTECTED]"

# JCE Installation
searchpath = default(
    'configurations/freeipa/searchpath',
    '/usr/lib/jvm/java-1.8*:/usr/lib/jvm/java-1.7*:/usr/jdk64/jdk1.7*:/usr/jdk64/jdk1.8*'
)
# folderpath="/jre/lib/security:/lib/security"
folderpath = default('configurations/freeipa/folderpath',
                     '/jre/lib/security:/lib/security')

admin_user_shell = default('configurations/freeipa/admin_user_shell',
                           '/sbin/nologin')

install_distribution_user = default(
    'configurations/freeipa/install_distribution_user', 'root')
