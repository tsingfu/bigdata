...
class RobotAdmin():
    ...
    def distribute_password(self, all_hosts=None, user="root"):
        ...
        if all_hosts is None:
            ...
            if (not len(ambhost)) or (ambhost == 'localhost' or
                                      ambhost.startswith(_hostname) or
                                      _hostname.startswith(ambhost)):
        ...
        new_hosts = [
            ...
            if host not in previously_distributed_hosts
        ]
        ...
        for new_host in new_hosts:
            subprocess.call([
                'scp', '-o', 'StrictHostKeyChecking=no', '-i', '%s/.ssh/id_rsa'
                % home, self.password_file, '%s@%s:%s' % (user, new_host, dest)
            ])
    ...
    def client_install(self,
        ...
        while not os.path.exists(self.password_file) \
            ...
            subprocess.Popen(
                [
                    "cp",
                    "/home/%s/%s" % (install_user, self.password_file_simple),
                    self.password_file
                ],
        ...
        if os.path.isfile(self.password_file):
            ...
            options = [
                '--enable-dns-updates', '--force-join', '--ssh-trust-dns',
                '--domain', domain, '--hostname', _hostname, '--realm', realm
            ] if install_with_dns else [
                '--force-join', '--hostname', _hostname
            ]

            # Install the ipa-client software, This requires the robot-admin password.
            p1 = subprocess.Popen(
                ['cat', self.password_file], stdout=subprocess.PIPE)
            p2 = subprocess.Popen(
                [
                    'ipa-client-install', '--mkhomedir', '--principal',
                    self.login, '-W', '--server', server, '-U'
                ] + options,
                stdin=p1.stdout,
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE)
            ...
            raise Exception('No robot-admin password could be found in %s ' %
                            self.password_file)
    ...
    def _all_hosts_local(self):
        ...
        if not len(password):
            raise ValueError(
                'Error reading ambari database password from file')
        protect(password)
        env = os.environ.copy()
        ...
        # the ambari server.
        p = subprocess.Popen(self.psql_query, stdout=subprocess.PIPE, env=env)
    ...
    def _all_hosts_remote(self, remote_machine):
        ...
        p = subprocess.Popen(
            ssh + ['cat ' + self.ambari_db_password_file],
            stdout=subprocess.PIPE)
        ...
        p = subprocess.Popen(
            ssh + [
                'bash -c "' + 'export PGPASSWORD=`cat ' +
                self.ambari_db_password_file + '`; ' +
                ' '.join(self.psql_query[:-1]) + ' \\"' + self.psql_query[-1] +
                '\\";' + '"'
            ],
            stdout=subprocess.PIPE)
...
class FreeIPACommon(object):
    ...
    def create_user_principal(self,
                              identity,
                              firstname=None,
        ...
        if not self.user_exists(identity):
            if firstname is None:
                firstname = identity
            subprocess.call([
                'ipa', 'user-add', '--first', firstname, '--last', lastname,
                identity
            ])
    ...
    def create_sudorule(self, rulename, **kwargs):
        ...
        if not self.sudorule_exists(rulename):
            cmd = ['ipa', 'sudorule-add', rulename] + [
                '--' + k + '=' + v for k, v in kwargs.iteritems()
                if k != "Users" and k != "Groups"
            ]
            subprocess.call(cmd)
            if "Users" in kwargs and len(kwargs["Users"]):
                subprocess.call([
                    'ipa', 'sudorule-add-user', '--users=' + ','.join(kwargs[
                        "Users"]), rulename
                ])
            if "Groups" in kwargs and len(kwargs["Groups"]):
                subprocess.call([
                    'ipa', 'sudorule-add-user',
                    '--groups=' + ','.join(kwargs["Groups"]), rulename
                ])
        else:
    ...
    def create_group(self, group, description, options=[]):
        if not self.group_exists(group):
            subprocess.call(
                ['ipa', 'group-add', group, '--desc', description] + options)

    def create_keytab(self, server, principal, realm, file, user, group,
                      permissions):
        ...
        subprocess.call(
            ['ipa-getkeytab', '-s', server, '-p', full_principal, '-k', file])
        ...
        try:
            with open(os.devnull, "w") as devnull:
                subprocess.check_call(
                    ['ipa', 'user-show', user], stderr=devnull)
            return True
    ...
    def sudorule_exists(self, sudorule):
        try:
            with open(os.devnull, "w") as devnull:
                subprocess.check_call(
                    ['ipa', 'sudorule-show', sudorule], stderr=devnull)
            return True
        except (subprocess.CalledProcessError, IOError, OSError):
            pass
        return False
    ...
    def service_exists(self, service):
        try:
            with open(os.devnull, "w") as devnull:
                subprocess.check_call(
                    ['ipa', 'service-show', service], stderr=devnull)
            return True
        except (subprocess.CalledProcessError, IOError, OSError):
            pass
        return False
    ...
    def group_exists(self, group):
        try:
            with open(os.devnull, "w") as devnull:
                subprocess.check_call(
                    ['ipa', 'group-show', group], stderr=devnull)
            return True
        except (subprocess.CalledProcessError, IOError, OSError):
            pass
        return False
    ...
    def group_add_member(self, group, user):
        subprocess.call(
            ['ipa', 'group-add-member', group, '--users=%s' % user])
    ...
    def update_password(self, user, password, password_file=None):
        ...
        with os.fdopen(
                os.open(password_file, os.O_WRONLY | os.O_CREAT, 0600),
                'w') as handle:
            protect(password)
        ...
        p2 = subprocess.Popen(
            ['ipa', 'user-mod', user, '--password'], stdin=p1.stdout)
        p1.stdout.close()
        p2.communicate()
    ...
    def __enter__(self):
        with open(os.devnull, "w") as devnull:
            p1 = subprocess.Popen(
                ['cat', self.password_file], stdout=subprocess.PIPE)
            p2 = subprocess.Popen(
                ['kinit', self.principal], stdin=p1.stdout, stdout=devnull)
            p1.stdout.close()
            p2.communicate()
...
def create_user(user, groups=[], comment='', options=[]):
    ...
    try:
        ...
        if len(groups) == 0:
            ...
            subprocess.call([
                'useradd', '-g', groups[0], '-G', ','.join(groups[1:])
            ] + options + [user])
