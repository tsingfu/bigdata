...
class FreeipaServer(Script):
    ...
    def install(self, env):
        ...
        install_command = 'ipa-server-install -U  --realm="%s" \
            --ds-password="%s" --admin-password="%s" --hostname="%s"'% (params.realm, params.directory_password, admin_password, _hostname)
    ...
    def create_base_accounts(self, env, fi):
        ...
        File(
            "/tmp/bind_user.ldif",
            content=Template(
                "bind_user.ldif.j2", expiry_date=expiry_date),
            mode=0600)
    ...
    def reset_robot_admin_expire_date(self, env):
        ...
        rm = freeipa.RobotAdmin()
        expiry_date = (datetime.datetime.now() + datetime.timedelta(weeks=52)
