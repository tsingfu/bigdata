...
class Remote(object):
    ...
    def check(self, firsttime=True):
        ...
        if firsttime:
            sshopts = [
                ...
                "StrictHostKeyChecking=no", "-o", "PasswordAuthentication=no",
                "-o", "ConnectTimeout=3"
            ]
        out = self.run(
            "echo Hello World from $HOSTNAME", sshopts=sshopts, exit=False)
        if "Hello World" not in out or "HOSTNAME" in out:
            raise ValueError("Unable to contact machine " + self.user + "@" +
                             self.host +
                             "! or machine did not respond correctly")
        return True
    ...
    def cp(self, local, remote, sshopts=[]):
        ...
        if directory and "-r" not in sshopts:
            sshopts.append("-r")
        return popen(["scp"] + sshopts +
                     [local, self.user + "@" + self.host + ":" + remote])
...
def yield_config(filename):
    ...
    with open(filename) as fp:
        ...
        for line in fp:
            if len(line.strip()):
                yield dict((n, v)
                           for n, v in zip(topline, line.strip().split(',')))
...
def check_keytabdict_consistency(keytabs):
    ...
    tuples = [(k["host"], k["principal name"], k["keytab file path"])
              for k in keytabs]
    ...
    tuples = [(k["host"], k["keytab file path"], k["keytab file owner"],
               k["keytab file group"]) for k in keytabs]
    if len(set(tuples)) != len(tuples):
        print(
            "Warning: your kerberos csv calls for multiple different ownership"
    ...
    for keytab in keytabs:
        ...
        if not len(identity):
            raise NameError("I could not interpret the identity of " + str(
                keytab))
        if not len(realm):
            raise NameError("I could not interpret the realm of " + str(
                keytab))
...
def create_user_principals(keytabs, ipa):
    ...
    for keytab in keytabs:
        ...
        if keytab["principal type"] == "USER":
            groups = []
            if keytab["keytab file owner"] == identity and keytab[
                    "keytab file group"] != identity:
                groups.append(keytab["keytab file group"])
            # First create any groups if needed
            for group in groups:
                ipa.create_group(group,
                                 'automatic group for principals/keytabs')
    ...
    for keytab in keytabs:
        ...
        for remote in remotes:
            ...
            if group in user_princ:
                continue
            if keytab["keytab file group"] not in remote.run(
                    "cut -d: -f1 /etc/group"):
        ...
        for remote in remotes:
            for user in users:
                ...
                if keytab["keytab file group"] not in remote.run("groups " +
                                                                 user):
                    remote.run("usermod -a -G " + keytab["keytab file group"] +
                               " " + user)
    return
...
def create_and_copy_local_keytabs(keytabs, ipa):
    ...
    for keytab in keytabs:
        ...
        intermediate_file = keytab["keytab file path"] + identity.replace('/',
                                                                          '_')
        ...
        if intermediate_file not in already_created:
            ...
            if (identity in ["HTTP/" + popen("hostname -f")] and
                    os.path.exists('/etc/httpd/conf/ipa.keytab')):
                # Special case for HTTP, needed by the FreeIPA server!
                popen("cp /etc/httpd/conf/ipa.keytab " + intermediate_file)
                popen("chown " + keytab["keytab file owner"] + ":" + keytab[
                    "keytab file group"] + " " + intermediate_file)
                popen("chmod " + keytab["keytab file mode"] + " " +
                      intermediate_file)
            else:
                ipa.create_keytab(
                    popen("hostname -f"),
        ...
        remote = Remote(keytab["host"])
        exists = remote.run("bash -c 'if [-d " + os.path.dirname(keytab[
            "keytab file path"]) + " ]; then echo \"yes\"; fi; '")
        exists = 'yes' in exists
        if not exists:
            remote.run("mkdir -p " + os.path.dirname(keytab[
                "keytab file path"]))

        remote.cp(intermediate_file, keytab["keytab file path"])
        remote.run("chown " + keytab["keytab file owner"] + ":" + keytab[
            "keytab file group"] + " " + keytab["keytab file path"])
        remote.run("chmod " + keytab["keytab file mode"] + " " + keytab[
            "keytab file path"])
...
def search_failed_keytabs(keytabs):
    ...
    for keytab in keytabs:
        ...
        try:
            identity = keytab["principal name"].split('@')[0]
            remote.run("su " + keytab["keytab file owner"] +
                       " bash -c '/usr/bin/kinit -kt " + keytab[
                           "keytab file path"] + " " + identity +
                       "; kdestroy;'")
        except OSError as e:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        raise AttributeError(
            "Supply the name of the kerberos keytabs csv file")
    # simple test that this works
    ipa = freeipa.FreeIPACommon()
    if not ipa.group_exists('admins'):
        raise SystemError(
            "No ipa group admins found ... did you forget to kinit or are you"
    ...
    user_princ = [
        keytab["principal name"].split('@')[0] for keytab in keytabs
        if keytab["principal type"] == "USER"
    ]
    # build a list of groups which are going to be created first
    group_princ = [
        keytab["keytab file group"].split('@')[0] for keytab in keytabs
        if keytab["principal type"] == "USER" and keytab["keytab file group"]
        != keytab["principal name"].split('@')[0]
    ]
    # Create user principals first
    ...
    if len(failed):
        ...
        raise RuntimeError("Failed to create some keytabs! " + str(
            len(failed)) + "/" + str(len(keytabs)))
    else:
