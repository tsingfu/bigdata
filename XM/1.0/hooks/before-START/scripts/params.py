...
if 'hadoop-env' in config['configurations']:
    ...
    hadoop_libexec_dir = config['configurations']['hadoop-env'][
        'install_dir'] + '/libexec'
    hadoop_lib_home = config['configurations']['hadoop-env'][
        'install_dir'] + '/lib'
    hadoop_bin = config['configurations']['hadoop-env'][
        'install_dir'] + '/sbin'
...
java_exec = "{0}/bin/java".format(
    java_home) if java_home is not None else "/bin/java"
...
mysql_driver_symlink_url = format(
    "{ambari_server_resources_url}/mysql-jdbc-driver.jar")
...
yarn_log_dir_prefix = default("/configurations/yarn-env/yarn_log_dir_prefix",
                              "/var/log/hadoop-yarn")
...
if command_params is not None:
    refresh_topology = bool(command_params[
        "refresh_topology"]) if "refresh_topology" in command_params else False

ambari_libs_dir = "/var/lib/ambari-agent/lib"

if 'hdfs-site' in config['configurations']:
    is_webhdfs_enabled = config['configurations']['hdfs-site'][
        'dfs.webhdfs.enabled']
...
if dfs_ha_enabled:
    ...
    namenode_rpc = default(
        '/configurations/hdfs-site/dfs.namenode.rpc-address', None)
...
if dfs_ha_enabled:
    ...
    dfs_lifeline_rpc_address = default(
        format('/configurations/hdfs-site/dfs.namenode.lifeline.rpc-address'),
        None)
