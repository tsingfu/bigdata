  ...
  """
    ...
    if params.has_namenode or params.dfs_type == 'HCFS':
        ...
        if os.path.exists(params.hadoop_conf_dir):
            if not os.path.exists(params.net_topology_script_dir):
                Directory(
                    params.net_topology_script_dir,
                    create_parents=True,
                    owner=params.hdfs_user)
            File(
            ...
            log4j_filename = os.path.join(params.hadoop_conf_dir,
                                          "log4j.properties")
            if (params.log4j_props != None):
                File(
                    ...
                    format("{params.hadoop_conf_dir}/log4j.properties"))):
                File(
                    log4j_filename,
                    mode=0644,
                    group=params.user_group,
...
def create_microsoft_r_dir():
    ...
    if 'MICROSOFT_R_NODE_CLIENT' in params.component_list and params.default_fs:
        ...
        try:
            ...
            Logger.warning(
                "Could not check the existence of {0} on DFS while starting {1}, exception: {2}".
                format(directory, params.current_service, str(exception)))
