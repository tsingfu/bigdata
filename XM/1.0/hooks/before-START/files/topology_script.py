...
DATA_FILE_NAME = os.path.dirname(os.path.abspath(
    __file__)) + "/topology_mappings.data"
...
class TopologyScript():
    ...
    def get_racks(self, rack_map, args):
        if len(args) == 1:
            ...
            return join([self.lookup_by_hostname_or_ip(input_argument,
                                                       rack_map)
    ...
    def lookup_by_hostname_or_ip(self, hostname_or_ip, rack_map):
        ...
        return rack if rack is not None else rack_map.get(
            "localhost.localdomain", DEFAULT_RACK)
