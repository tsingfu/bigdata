...
from ranger_functions import Rangeradmin
...
from install_ranger_plugin import install_ranger_plugin
...
def setup_ranger_plugin(component_select_name, service_name, previous_jdbc_jar,
                        ...
                        cred_setup_prefix_override=None, only_download=False):
                ...
                if os.path.isfile(policycache_json_file) and os.path.getsize(policycache_json_file) > 0:
                    with open(policycache_json_file) as json_file:
                        ...
                        if 'serviceName' in json_data and json_data['serviceName'] == repo_name:
                            ...
                            Logger.warning(
                                "If service name for {0} is not created on Ranger Admin UI, then to re-create it delete policy cache file: {1}".format(
    ...
    for jar_file in jar_files:
        for component in component_list:
            Execute(('ln', '-sf', format('{stack_root}/ranger-{service_name}-plugin/lib/{jar_file}'),
                    ...
                    not_if=format('ls {stack_root}/{component}/lib/{jar_file}'),
...
def setup_ranger_plugin_keystore(service_name, audit_db_is_enabled, stack_version, credential_file,
    ...
    if cred_setup_prefix_override is not None:
        ...
        cred_setup_prefix = (
            format('{stack_root}/ranger-{service_name}-plugin/ranger_credential_helper.py'), '-l', cred_lib_path)
