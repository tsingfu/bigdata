...
admin_username = default("/configurations/ranger-env/admin_username", "admin")
admin_password = default("/configurations/ranger-env/admin_password", "admin")
from resource_management.core.logger import Logger

try:
    Logger.info('ranger admin user  is %s, ranger admin password is %s' % (admin_username, admin_password))
except Exception as e:
    print 'ranger admin user  is %s, ranger admin password is %s' % (admin_username, admin_password)
...
def curl_krb_request(tmp_dir, keytab, principal, url, cache_file_prefix,
    ...
    ccache_file_path = "{0}{1}{2}_{3}_cc_{4}".format(curl_krb_cache_path, os.sep, cache_file_prefix, user,
                                                     ccache_file_name)
    ...
    try:
        ...
        if is_kinit_required:
            ...
            shell.checked_call("{0} -c {1} -kt {2} {3} > /dev/null".format(kinit_path_local,
                                                                           ccache_file_path, keytab, principal),
                               user=user)
    ...
    try:
        if return_only_http_code:
            _, curl_stdout, curl_stderr = get_user_call_output(
                ['curl', '--location-trusted'] + ssl_options + ['-u', admin_username + ':' + admin_password, '-b',
            ...
            curl_command = ['curl', '--location-trusted'] + ssl_options + ['--negotiate', '-u',admin_username + ':' + admin_password, '-b', cookie_file,
                                                                           '-c', cookie_file,
                                                                           url, '--connect-timeout',
                                                                           str(connection_timeout), '--max-time',
                                                                           str(maximum_timeout)]
